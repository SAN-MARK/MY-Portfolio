...
export const SelectedWorkSection: React.FC<SelectedWorkSectionProps> = ({
  ...
  const filterCategories = ['ALL', 'FULL STACK', 'UI/UX', 'AI'];

  const filteredProjects = PROJECTS_DATA.filter((project) => {
    if (activeFilter === 'ALL') return true;
    if (activeFilter === 'FULL STACK') {
      return project.tags.includes('REACT') || project.tags.includes('VERCEL') || project.tags.includes('JS') || project.tags.includes('DATABASE');
    }
    if (activeFilter === 'UI/UX') {
      return project.tags.includes('FIGMA') || project.category.includes('Academic') || project.tags.includes('CSS');
    }
    if (activeFilter === 'AI') {
      return project.tags.includes('AI') || project.description.toLowerCase().includes('ai');
    }
    return true;
  });
